...
export default function ClassDetail() {
  ...
  const { data, loading, error } = useQuery(GET_CLASS_DATA, {
    variables: { id },
    fetchPolicy: "cache-and-network",
  });

  const [createSub, { loading: creating }] = useMutation(CREATE_SUBJECT, {
    update(cache, { data: { createSubject } }) {
      ...
      cache.writeQuery({
        ...
        data: {
          ...existing,
          classGroupById: {
            ...existing.classGroupById,
            teachingassignmentSet: [
              ...existing.classGroupById.teachingassignmentSet,
              newAssignment,
            ],
          },
        },
  ...
  if (loading)
    return (
      <div className="h-screen animate-pulse flex items-center justify-center bg-slate-50 font-black text-slate-300 text-xl uppercase tracking-widest">
        Sincronizando Grade...
      </div>
    );
  ...
  const hasSubjectInClass = turma?.teachingassignmentSet?.some(
    (a) => a.teacher.id === user?.id
  );
  ...
  const handleCreate = async () => {
    ...
    if (!subName.trim()) {
      Swal.fire("Atenção", "Digite o nome da disciplina.", "warning");
      return;
    }
            ...
            <div className="flex items-center gap-6">
              <button
                onClick={() =>
                  navigate(
                    turma?.course?.id
                      ? `/courses/${turma.course.id}/classes`
                      : "/courses"
                  )
                }
              ...
              <div>
                <h2 className="text-4xl font-black text-slate-800 tracking-tight italic leading-tight">
                  {turma?.name}
                </h2>

                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">
                  Disciplinas Ativas na Unidade
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {turma?.teachingassignmentSet.map((a) => (
              <div
                ...
                onClick={() =>
                  navigate(
                    `/classes/${id}/subject/${a.subject.id}?teacherId=${a.teacher.id}`
                  )
                }
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-2xl group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner">
                    📚
                  </div>
                ...
                <p className="text-slate-400 font-bold text-sm">
                  {user?.id === a.teacher.id
                    ? "Minha Disciplina"
                    : `Prof. ${a.teacher.firstName}`}
                </p>

                <div className="absolute -right-4 -bottom-4 text-7xl opacity-[0.02] font-black italic -rotate-12 select-none">
                  SUBJECT
                </div>
              </div>
            ))}
          ...
          {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
              <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl animate-in zoom-in duration-200">
                <h3 className="text-2xl font-black mb-1 text-slate-800 italic">
                  Sua Disciplina
                </h3>

                <p className="text-slate-400 text-[10px] mb-8 uppercase font-black tracking-widest">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-4 bg-slate-100 text-slate-400 rounded-2xl font-bold"
                    >
                      Cancelar
                    </button>
                  </div>
