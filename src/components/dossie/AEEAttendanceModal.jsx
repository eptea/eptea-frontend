    ...
    const result = await Swal.fire({
      ...
      html: `
        <div style="font-size: 14px; line-height: 1.6;">
          ...
          <br><br>
          <p>Registrado por: <strong>${specialistName}</strong></p>
          <br>
      ...
      .map((record) => {
        ...
        return `
          <tr>
            ...
            <td style="font-weight: bold;">${specialistName.toUpperCase()}</td>
            <td>
              <div style="border-bottom: 1px solid #ccc; height: 30px; margin-top: 10px;"></div>
            </td>
          </tr>
        `;
      })
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .sig-box { width: 30%; border-top: 1px solid #000; padding-top: 5px; }
                ...
                {Object.entries(groupedByMonth).map(([monthYear, records]) => (
                  <div key={monthYear} className="bg-slate-50 rounded-3xl p-6 border border-slate-100">
                    ...
                    <div className="space-y-3">
                      {records.map((record) => {
                        // Verifica se o usuário logado é o autor do registro
                        const isOwner = currentUser?.id && String(record.aeeSpecialist?.id) === String(currentUser.id);

                        return (
                          <div key={record.id} className="bg-white p-4 rounded-2xl shadow-sm flex items-center justify-between gap-4">
                            <div className="flex items-start gap-4 flex-1">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-[10px] text-slate-500 mt-1">
                                  Profissional: <span className="font-bold">{record.aeeSpecialist?.firstName} {record.aeeSpecialist?.lastName}</span>
                                </p>
                              </div>
                            </div>

                            {/* Botão de Deletar renderizado condicionalmente caso seja o criador */}
                            {isOwner && (
                              <button
                                onClick={() => handleDelete(record)}
                                title="Excluir registro"
                                className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white p-2.5 rounded-xl transition-colors"
                              >
                                🗑️
                              </button>
                            )}
