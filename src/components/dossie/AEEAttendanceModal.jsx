    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
      <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="bg-indigo-600 p-6 flex justify-between items-center text-white">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 bg-white/20 rounded-full font-black hover:bg-white/40 transition-colors"
          >
            ✕
          </button>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl border-none shadow-inner focus:ring-2 focus:ring-indigo-500 font-bold text-slate-700"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl border-none shadow-inner focus:ring-2 focus:ring-indigo-500 text-sm resize-none"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  rows="2"
                  value={form.meth}
                  onChange={(e) => setForm({ ...form, meth: e.target.value })}
                  className="w-full p-3 rounded-xl border-none shadow-inner focus:ring-2 focus:ring-indigo-500 text-sm resize-none"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  rows="2"
                  value={form.perf}
                  onChange={(e) => setForm({ ...form, perf: e.target.value })}
                  className="w-full p-3 rounded-xl border-none shadow-inner focus:ring-2 focus:ring-indigo-500 text-sm resize-none"
                  required
                />
              </div>
              <button
                type="submit"
                disabled={saving}
                className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-lg hover:bg-indigo-700 disabled:bg-slate-300"
              >
                ...
                {Object.entries(groupedByMonth).map(([monthYear, records]) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-4 border-b border-slate-200 pb-4">
                      <h5 className="font-black text-slate-700 uppercase tracking-widest">
                        {monthYear}
                      </h5>
                      <button
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-[10px] text-slate-500 mt-1 line-clamp-1">
                              Resp: {record.aeeSpecialist?.firstName}
                            </p>
                          </div>
                        </div>
                      ))}
